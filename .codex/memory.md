# DesktopVoiceInput Project Memory
...
- The project has an archive snapshot on branch `codex/archive-current-state` at commit `84db14e`.
...
- Add stronger guards so recording, settings editing, and text insertion do not interfere with one another.
...
- Packaging, distribution, and install flow are not set up yet.
...
3. Add clearer validation and error handling for Doubao and Qwen configuration.
